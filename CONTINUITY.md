...
| 题目区行高 | 1.65 | |
| 分析区字号 | 32px | LikeJianJianTi（手写尖尖体），红色 |
| 解答区字号 | 32px | LikeJianJianTi（手写尖尖体），黑色 |
| 总结区字号 | 32px | LikeJianJianTi（手写尖尖体），黑色 |
| 其他区行高 | 微小随机（约1.6-1.8） | 不用管，按正常行高估算 |

**行高估算公式**（用于计算起手坐标的 y 增量）：
`  `
每行高度(px) ≈ 字号 × 行高
每行高度(%) = 每行高度(px) / 980 × 100
`  `
示例：32px 字号 × 1.7 行高 = 54.4px ≈ 5.55% 画布高度。即分析区每写一行，y 坐标约增加 5.5%。
