...
async function captureAndSaveScreenshot() {
  ...
  try {
    ...
    if (result.ok) {
      ...
      console.warn('截图存档失败：', result.error)
...
async function confirmStep1() {
  ...
  // screenshotUrl 来自 planBoardLabels 时的截图存档(1.3)，不发给 B，只在本地文件存档
  ...
  try {
    ...
    if (result.ok) {
      ...
      message.error(`题目信息保存失败：${result.error || '未知错误'}，请重试`)
    }
  } catch (error) {
      ...
      <a-row :gutter="[16, 16]" style="margin-top: 0">
        <a-col :xs="24">
          <a-card title="知识关联点" class="knowledge-card qh-surface-card" :bordered="false">
            ...
            <a-divider />
            <a-space wrap>
              <a-button type="primary" :disabled="!canConfirm" @click="confirmStep1">
      ...
      <a-form layout="vertical" class="agent-form">
        ...
        <a-space wrap>
          <a-tag :color="isUserApiReady() ? 'success' : 'default'">
            {{ isUserApiReady() ? 'Agent A 可调用' : '请填完整接口地址、API Key、模型名称' }}
          </a-tag>
        </a-space>
        ...
        <a-form-item label="接口地址（仅支持 OpenAI 兼容协议 API）">
          <a-input v-model:value="agentBApiConfig.endpoint" placeholder="https://api.example.com/v1/chat/completions" />
        </a-form-item>
        <a-form-item label="API Key">
          <a-input-password
        ...
        <a-tag :color="isAgentBApiReady() ? 'success' : 'default'">
          {{ isAgentBApiReady() ? 'Agent B 可调用' : '请填完整接口地址、API Key、模型名称' }}
        </a-tag>
        ...
        <div class="param-subtitle">自定义 OpenAI 兼容模型；接口地址必须是完整 Chat Completions URL。</div>
        <a-form-item label="接口地址（仅支持 OpenAI 兼容协议 API）">
          <a-input v-model:value="checkAgentApiConfig.endpoint" placeholder="https://api.example.com/v1/chat/completions" />
        </a-form-item>
        <a-form-item label="模型名称">
        ...
        <a-tag :color="isCheckAgentApiReady() ? 'success' : 'default'">
          {{ isCheckAgentApiReady() ? 'Check Agent 可调用' : '请填完整接口地址、API Key、模型名称' }}
        </a-tag>
      </a-form>
      <template #footer>
