...
export const ROUGH_NOTATION_ENABLED_ACTIONS = Object.freeze(['underline', 'highlight'])
...
function resolveColor(colorId) {
  const color = BOARD_MARK_COLORS[colorId]
  if (!color) throw new Error(`未知颜色 ID: ${colorId || '空'}`)
  return color
}
...
function normalizeOptions(action, options = {}) {
  ...
  if (options.colorId && options.colorId !== colorId) {
    throw new Error(`${action} 颜色固定为 ${colorId}`)
  }
  return {
    type: action,
    animate: true,
